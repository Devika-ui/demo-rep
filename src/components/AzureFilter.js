...
const AzureFilter = ({ additionalFilters = [] }) => {
  ...
  useEffect(() => {
    // Fetch initial filters on component mount
    fetchInitialFilters();
  }, []);
  ...
  const handleFilterChange = async (filterType, values) => {
    ...
    // Fetch filters based on all selected filters
    const filterParams = {
    ...
    await fetchFiltersBasedOnSelection(filterParams);
  };
